    ...
    for conf in configs:
        ...
        for i in range(nrepetitions):
            ...
            for j in range(nfolds):
                ...
                X_ = np.delete(X, val_ind, axis=0)
                y_ = np.delete(y, val_ind, axis=0)
                X_val = X[val_ind]
                y_val = y[val_ind]
...
class krr():
    ...
    def getTrainK(self, X):
        if self.kernel == 'linear':
            ...
            return (X@X.T + 1) ** self.kernelparameter
        elif self.kernel == 'gaussian':
            dist = (-2 * X @ X.T + (np.sum(X ** 2, axis=1) + np.sum(X ** 2, axis=1)[:, np.newaxis]).T)**0.5
            return (np.exp(-1*((dist)**2)/(2 * np.square(self.kernelparameter))))
        else:
            print("Faulty Kernel")
    ...
    def getPredK(self, X):
        if self.kernel == 'linear':
            ...
            return (X@self.trainX.T + 1) ** self.kernelparameter
        elif self.kernel == 'gaussian':
    ...
    def predict(self, X):
        return self.getPredK(X)@self.alpha
